...
def test_socket_send(testdir):
    ...
    with mock.patch("socketio.Client") as mock_class:
        ...
        assert mock_class.mock_calls == [
            ...
            mock.call().emit('testreport',
                {'id': 'test_socket_send.py::test_one', 'when': 'setup', 'outcome': True}),
            mock.call().emit('testreport',
                {'id': 'test_socket_send.py::test_one', 'when': 'call', 'outcome': True}),
            mock.call().emit('testreport',
                {'id': 'test_socket_send.py::test_one', 'when': 'teardown', 'outcome': True}),
            mock.call().emit('testreport',
                {'id': 'test_socket_send.py::test_two', 'when': 'setup', 'outcome': True}),
            mock.call().emit('testreport',
                {'id': 'test_socket_send.py::test_two', 'when': 'call', 'outcome': True}),
            mock.call().emit('testreport',
                {'id': 'test_socket_send.py::test_two', 'when': 'teardown', 'outcome': True}),
            mock.call().__bool__(),
